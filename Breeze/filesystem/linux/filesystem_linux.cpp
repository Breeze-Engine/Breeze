#include "Breeze/filesystem/linux/filesystem_linux.h"
#include "Breeze/utility/assert.h"
#include "Breeze/utility/error_helper.h"
#include "Breeze/utility/path_helper.h"
#include <sys/stat.h>
#include <sys/types.h>
#include <unistd.h>
#include <algorithm>
...
namespace breeze::detail::linux {

    std::unique_ptr<Error> CreateNewDirectory(const std::string& path) noexcept {
        if (path.empty()) {
    ...
    std::unique_ptr<Error> CreateDirectories(const std::string& path) noexcept {
        if (path.empty()) {
            return errors::New("path is empty");
        }
    ...
    std::tuple<std::size_t, std::unique_ptr<Error>> GetFileSize(const std::string& path) noexcept {
        struct ::stat st;
        if (::stat(path.data(), &st) != 0) {
            auto err = detail::ToErrc(errno);
    ...
    std::tuple<std::string, std::unique_ptr<Error>> GetLocalAppDataDirectoryPath() noexcept {
        return std::make_tuple("", errors::New("not implemented yet"));
    }

    std::tuple<std::string, std::unique_ptr<Error>> GetAppDataDirectoryPath() noexcept {
        return std::make_tuple("", errors::New("not implemented yet"));
    }

    std::tuple<std::string, std::unique_ptr<Error>> GetResourceDirectoryPath() noexcept {
    ...
    std::tuple<std::string, std::unique_ptr<Error>> GetTempDirectoryPath() noexcept {
        return std::make_tuple(P_tmpdir, nullptr);
    }

    std::tuple<std::string, std::unique_ptr<Error>> GetCurrentWorkingDirectory() noexcept {
        char dir[PATH_MAX];
